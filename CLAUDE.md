...
New budget section showing countries extracted from HLO `hlo_geo` field per center+AOW. Users input a percentage per country; budget is computed on the fly as `percentage × pooledTotal / 100` (not stored in DB). Entity `porb_country_percentage` stores: program_id, porb_aow_id, center_id, country_name, percentage, is_manual. Per Rule 15, the total percentage per center+AOW must equal exactly 100% — or 0% if the section is unused; any other total is a validation error (red badge + warning text + AOW/center error icons + blocks submit). The input value itself is never clamped or reset — users can type anything and it's saved as-is; only the total is validated. Consolidated views recalculate percentage as `country_budget / total_pooled_funding × 100`. Summary detailed view aggregates by country across centers. Section disabled when no countries in HLO data, EXCEPT for AOW00 (Cross-Cutting) which is always enabled. For AOW00, users can manually add countries via CLARISA search dialog (`GET /porb/country-percentage/search-clarisa?q=...`, `POST /porb/country-percentage`, `DELETE /porb/country-percentage/:id`). Manual countries have `is_manual: true` and can be deleted; HLO-derived countries cannot. Validation only counts "live" rows (HLO-derived for current geo, or `is_manual`) so orphan rows don't fire phantom errors.

### Location of Benefit Section
New budget section (after Countries of Implementation in nav order) showing locations extracted from outcome-level `outcome_geo` field per center+AOW. Unlike Countries Percentage (countries only from HLOs), this supports **country, region, and Global** location types from **outcomes**. Entity `porb_location_benefit` stores: program_id, porb_aow_id, center_id, location_name, location_type ('country'|'region'|'global'), percentage, is_manual. `porb_outcome.outcome_geo` column (mediumtext, nullable) stores the parsed geo string from TOC import (`'Global'`, `'Region: X, Y'`, `'Country: X, Y'`). Same percentage logic as Countries: budget computed as `percentage × pooledTotal / 100`; per Rule 16, total per center+AOW must equal exactly 100% or 0% (any other total is a validation error that blocks submit), input value is never clamped/reset. Consolidated endpoint: `GET /porb/location-benefit-consolidated`. Manual add dialog exists but is currently hidden (commented out in template). Included in: submission snapshots (`location_benefits` per center), version view, Excel export ("Location of Benefit" sheet), `clearAllPorbData`, socket events (`section: 'location-benefit'`).
...
### Programs List "Last update on" Derives from History
The Programs list `last_update_at` column is **not** the stored `initiative.last_update_at` value — `findAllFull` in `initiatives.service.ts` overrides it with the `createdAt` of the latest joined `history` row, and the Draft filter compares `latest_history.createdAt > last_submitted_at`. The stored field is still written on submit (set to the same `createdAt` as the submit's history row so the equality check holds at submit time), but every other mutation relies on the derived value. Practical consequence: any PORB mutation that should bump "Last update on" MUST call `logHistory(...)`. The create branches of `updateCross`, `updateCountryPercentage`, `updateLocationBenefit`, plus `createUnknownPartner`, `resolveUnknownPartner`, `deleteUnknownPartner`, and the three `deleteTocDeleted*` handlers all log history for this reason. If you add a new mutation and skip `logHistory`, the row will save but the Programs list timestamp won't move.
