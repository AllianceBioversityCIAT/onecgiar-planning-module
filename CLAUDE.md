...
### Summary View Modes
The summary tab has a top-level Consolidated/Detailed toggle. Consolidated shows the main table + Anaplan + W3 totals. Detailed shows AOW nav + section nav + per-AOW budget breakdown. W3/Bilateral has its own button in the detailed AOW nav. `summaryViewMode` controls the toggle. Zero-budget AOW rows are hidden from the consolidated table. Sections with no budget data are disabled (dimmed, not clickable) with tooltip. Selected section is preserved when switching AOWs.

### Summary Assumption Icon
`SummaryAssumptionIconComponent` opens a click-to-view dialog (not tooltip) with assumption text. Accepts single `[assumption]` string or `[assumptions]` array of `{center, assumption}` entries for multi-center items. `SummaryAssumptionDialogComponent` renders the dialog. For grouped HLO totals, shows all center assumptions in one dialog. Backend enriches HLO, MELIA, and bilateral rows with `center_name` in `getSummaryAowDetail()`.
