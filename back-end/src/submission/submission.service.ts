...
export class SubmissionService {
  ...
  async updateCenterStatus(data, reqUser) {
    const { initiative_id, organization_code, phase_id, status, organization } =
      data;
    ...
    await this.centerStatusRepo.save(center_status).then(
      async (data) => {
        ...
        history.resource_property = data.status
          ? 'Mark as complete'
          : 'Mark as incomplete';
  ...
  async updateCenterValidate(data, reqUser) {
    ...
    await this.centerStatusRepo.save(center_status).then(
      async (data) => {
        ...
        const history = this.historyRepository.create();
        history.resource_property = data.is_valid
          ? 'Mark as valid'
          : 'Mark as invalid';
        history.user_id = reqUser.id;
        history.initiative_id = data.initiative_id;
      ...
      async () => {
        ...
        history.resource_property =
          data.status == 'Approved'
            ? `Approved for version Id: ${id}`
            : `Rejected for version Id: ${id}`;
  ...
  async createNew(user_id, initiative_id, phase_id, json, tocSubmissionData) {
    try {
      ...
      const phaseObject = await this.phaseRepository.findOneBy({
        id: phase_id,
      });
  ...
  async getSelectedCountry(
    ...
    return await this.partnerCountryRepository
      ...
      .addSelect(
        'GROUP_CONCAT(country.name ORDER BY country.name)',
        'countries',
      )
    ...
    if (organizationObject != null) {
      ...
      await this.resultValuesRepository.save(newResultPeriodValue).then(
        async (data) => {
          ...
          history.resource_property = value
            ? 'Checked partner'
            : 'unchecked partner';
          history.old_value =
            newResultPeriodValue.value == true ? 'False' : 'True';
  ...
  async saveAllResultData(id, data: any, user) {
    ...
    if (!value) await this.clearAllResultValues(id, data);
    return { message: 'Data saved' };
  }
  async clearAllResultValues(id: number, data: any) {
    const initiativeId = id;

    const { partner_code, wp_id, itemsIds, phase_id } = data;
    const initiativeObject = await this.initiativeRepository.findOneBy({
      id: initiativeId,
    });
    let organizationObject = await this.organizationRepository.findOneBy({
  ...
  async saveResultDataValue(id, data: any, user) {
    ...
    let budgetAssumptions = await this.budgetAssumptionsService.findOne(
      budgetAssumptionsData,
    );
    ...
    if (oldResult) {
      ...
      Object.keys(objDifference).forEach(async (key) => {
        ...
        if (key == 'no_budget') {
          history.resource_property = value
            ? 'Checked result as no budget assigned'
    ...
    if (oldResult) {
      ...
      (oldResult.phase_id = phase_id), (oldResult.type = type);
  ...
  getDifference(a, b) {
    return Object.fromEntries(
      Object.entries(b).filter(
        ([key, val]) => key !== 'value' && key in a && a[key] !== val,
      ),
    );
  }
      ...
      await this.wpBudgetRepository.save(oldWpBudget).then(
        async (data) => {
          ...
          if (oldData.budget == '' && data.budget != '') {
            ...
            history.old_value = null;
            history.new_value =
              data.budget == '' ? '0' : Number(data.budget).toString();
          } else if (oldData.budget != '' && data.budget != '') {
            history.resource_property = 'Edit total budget';
            history.old_value =
              oldData.budget == '' ? '0' : Number(oldData.budget).toString();
            history.new_value =
              data.budget == '' ? '0' : Number(data.budget).toString();
          } else {
  ...
  getAllMeliasLength() {
    ...
    for (let wp of this.actualWps) {
      total += this.allData[wp.ost_wp.wp_official_code].filter(
        (d: any) => d.category == 'Melia',
      ).length;
    }
    return total;
  }
  ...
  getConsolidatedData(wps: any[], indicatorsTypes: any[]) {
    ...
    wps.forEach((wp: any) => {
      ...
      indicatorsTypes.forEach((indicator: any) => {
        obj[indicator] = this.getTotalIndValues(
          this.perAllValuesIndicator?.[wp.ost_wp.wp_official_code],
          indicator,
        );
      });
      obj['Melias'] = this.allData[wp.ost_wp.wp_official_code].filter(
        (d: any) => d.category == 'Melia',
      ).length;
      // obj['Percentage'] = this.sammaryTotalConsolidated[wp.ost_wp.wp_official_code] + '%';
      obj['Percentage'] = this.formatWithThousandsSeparator(
        this.roundNumber(
          this.summaryBudgetsTotal[wp.ost_wp.wp_official_code + '-project'],
        ),
      );
      obj['Budgets'] = this.formatWithThousandsSeparator(
    ...
    indicatorsTypes.forEach((indicator: any) => {
      obj[indicator] = this.getTotalIndAllValues(
        this.perAllValuesIndicator,
        indicator,
      );
    });
  ...
  getConsolidatedDataForPartners(
    wps: any[],
    indicatorsTypes: any[],
    partner_code: number,
  ) {
    ...
    obj['Melias'] = this.getAllMeliasLength();
    (obj['Percentage'] = this.getTotalBudgetForEachPartnerProject(
      this.wp_budgets[partner_code],
    )),
  ...
  getAllData(wps: any[], indicatorTypes: any[]) {
    ...
    wps.forEach((wp: any) => {
      ...
      data = this.allData[wp.ost_wp.wp_official_code].map((d: any) => {
        ...
        obj['Budget_USD'] = this.formatWithThousandsSeparator(
          this.roundNumber(
            this.toggleSummaryValues[wp.ost_wp.wp_official_code],
          ),
        )
      ...
      obj['Budget_USD'] = this.formatWithThousandsSeparator(
        this.roundNumber(this.toggleSummaryValues[wp.ost_wp.wp_official_code]),
      )
        ? this.formatWithThousandsSeparator(
            this.roundNumber(
  ...
  async getAllDataForGeoAndPartner(
    ...
    const newArray: any[][] = Array(wps.length)
      .fill(null)
      .map(() => []);
    ...
    for (let i = 0; i < wps.length; i++) {
      ...
      const data = await Promise.all(
        dataList.map(async (d: any) => {
          let obj: any = {};
          obj['id'] = d.id;
          if (d.category == 'partners') obj['Partner'] = d.name;
          else obj['Scope'] = d.location;
          obj['Type'] = this.getCategory(d.category);
          ...
          for (const partner of partners) {
            for (const per of period) {
              const hasCenter =
                this.perValues?.[partner?.code]?.[wpCode]?.[d.id]?.[per.id] ===
                true;

              if (hasCenter) {
                const partnerCenters = await this.getPartners(
                  String(d.id),
                  submissionId,
                );
                if (partnerCenters) {
                  partnerCenters
                    .split(',')
                    .forEach((c) => centerSet.add(c.trim()));
                }
              }
            }
          }
  ...
  getPartnersData(
    ...
    partners = partners.sort((a: any, b: any) =>
      a?.acronym?.toLowerCase().localeCompare(b?.acronym?.toLowerCase()),
    );
    partners.forEach((partner: any) => {
      ...
      wps.forEach((wp: any) => {
        data = (
          this.partnersData[partner.code][wp?.ost_wp?.wp_official_code] || []
        )?.map((d: any) => {
          let obj: any = {};
          obj['id'] = d.id;
          obj['WP_Results'] = d?.initiativeMelia?.meliaType?.name
          ...
          // if(d.category == 'WP') {
          obj['Budget'] = this.formatWithThousandsSeparator(
            this.roundNumber(
              this.toggleValues[partner.code][wp.ost_wp.wp_official_code],
            ),
          )
            ? this.formatWithThousandsSeparator(
                this.roundNumber(
  ...
  async generateExcel(
    ...
    if (!this.initiative_data.synchronized)
      this.results = [...cross_data, ...this.ipsr_value_data, ...this?.results];
    else this.results = [...cross_data, ...this?.results];
    ...
    const isCrosscuttingAndManagementValid = this.wps.some(
      (wp: any) => wp.ost_wp?.acronym == 'AOW00',
    );

    if (
      !isCrosscuttingAndManagementValid &&
      this.initiative_data.synchronized
    ) {
      this.wps.unshift({
        id: 'CROSS',
    ...
    if (this.initiative_data.synchronized) {
      for (let wp of this.wps) {
        melias.push({
          ...
          category: 'melia',
          ost_wp: {
            wp_official_code: wp.ost_wp.wp_official_code + '-melia',
            acronym: wp.ost_wp.wp_official_code,
          },
        });
      }
    }
    ...
    if (this.initiative_data.synchronized) {
      for (let wp of this.wps) {
        crossCutting.push({
          ...
          category: 'Cross Cutting',
          ost_wp: {
            wp_official_code: wp.ost_wp.wp_official_code + '-Cross-Cutting',
          },
        });
      }
    }
    ...
    if (this.initiative_data.synchronized) {
      for (let wp of this.actualWps) {
        geographicScope.push({
          ...
          ost_wp: {
            wp_official_code: wp.ost_wp.wp_official_code + '-Geographic-Scope',
          },
        });
    ...
    if (this.initiative_data.synchronized) {
      for (let wp of this.actualWps) {
        synergyPrograms.push({
          ...
          ost_wp: {
            wp_official_code: wp.ost_wp.wp_official_code + '-synergy-programs',
          },
        });
      }
    }
    ...
    for (let partner of partners) {
      ...
      for (let wp of this.wps) {
        ...
        if (result.length) {
          ...
          const indicatorTogelvalue = await this.constantsRepository.findOne({
            where: { id: 3 },
          });

          const filterd_results = result.filter((r: any) =>
            r.category.includes('OUTPUT'),
          );
          if (filterd_results.length > 0 && indicatorTogelvalue?.value == '1') {
            const indicator_filterd = [
              ...result.filter((r: any) => !r.category.includes('OUTPUT')),
              ...filterd_results.filter((d: any) =>
                d?.pooled_centers
                  ?.map((d: any) => d.code)
                  .includes(partner.code),
              ),
            ];
            const updatedIndicators = indicator_filterd.map((res: any) => ({
              ...res,
              quantitative_indicators: (
                res.quantitative_indicators ?? []
              ).filter((i: any) =>
      ...
      if (!this.initiative_data.synchronized)
        if (this.partnersData[partner.code]?.IPSR)
          this.partnersData[partner.code].IPSR = this.partnersData[
            partner.code
          ]?.IPSR?.filter((d: any) => d.value != null && d.value != '').sort(
            (a: any, b: any) => +(a.ipsr.id - b.ipsr.id),
          );

      if (!this.initiative_data.synchronized) {
        let newCrossCenters = this.partnersData[partner.code]?.CROSS?.filter(
          (d: any) => d.category == 'Cross Cutting',
        ).sort((a: any, b: any) =>
          b?.title?.toLowerCase().localeCompare(a?.title?.toLowerCase()),
        );
        if (this.partnersData[partner.code]?.CROSS)
          this.partnersData[partner.code].CROSS = this.partnersData[
            partner.code
          ]?.CROSS?.filter((d: any) => d.category != 'Cross Cutting').sort(
            (a: any, b: any) =>
              a?.title?.toLowerCase().localeCompare(b?.title?.toLowerCase()),
          );
        newCrossCenters?.forEach((d: any) =>
          this.partnersData[partner.code].CROSS.unshift(d),
        );
      }

      this.wps.forEach((d: any) => {
        if (d.category == 'WP') {
          let outputData = this.partnersData[partner.code][
            d.ost_wp.wp_official_code
          ]
            .filter((d: any) => d.category == 'OUTPUT')
            .sort((a: any, b: any) =>
              a.title
                .replace(/[\s~`!@#$%^&*(){}\[\];:"'<,.>?\/\\|_+=-]/g, '')
                .toLowerCase()
          ...
          let outcomeData = this.partnersData[partner.code][
            d.ost_wp.wp_official_code
          ]
            .filter((d: any) => d.category != 'OUTPUT')
            .sort((a: any, b: any) =>
          ...
          this.partnersData[partner.code][d.ost_wp.wp_official_code] =
            outputData.concat(outcomeData);
        }
      });
    }
    ...
    //sort first AOW
    const newCROSS = this.allData[firstKey]
      .filter((d: any) => d.category == 'Cross Cutting')
      .sort((a: any, b: any) =>
        b?.title?.toLowerCase().localeCompare(a?.title?.toLowerCase()),
      );
    this.allData[firstKey] = this.allData[firstKey]
      .filter((d: any) => d.category != 'Cross Cutting')
      .sort((a: any, b: any) =>
        a?.title?.toLowerCase().localeCompare(b?.title?.toLowerCase()),
      );
    ...
    this.wps.forEach((d: any) => {
      if (d.category == 'WP') {
        let outputData = this.allData[d.ost_wp.wp_official_code]
          .filter((d: any) => d.category == 'OUTPUT')
          .sort((a: any, b: any) =>
            a.title
              .replace(/[\s~`!@#$%^&*(){}\[\];:"'<,.>?\/\\|_+=-]/g, '')
        ...
        let outcomeData = this.allData[d.ost_wp.wp_official_code]
          .filter((d: any) => d.category != 'OUTPUT')
          .sort((a: any, b: any) =>
            a?.title
              ?.replace(/[\s~`!@#$%^&*(){}\[\];:"'<,.>?\/\\|_+=-]/g, '')
        ...
        this.allData[d.ost_wp.wp_official_code] =
          outputData.concat(outcomeData);
      }
    });
    ...
    this.GeographicScopeWp = this.wps.filter(
      (d: any) => d.category == 'Geographic-Scope',
    );
  ...
  sammaryCalc() {
    ...
    Object.keys(this.displayBudgetValuesIndicator).forEach((partner_code) => {
      Object.keys(this.displayBudgetValuesIndicator[partner_code]).forEach(
        (wp_id) => {
          ...
          Object.keys(
            this.displayBudgetValuesIndicator[partner_code][wp_id],
          ).forEach((item_id) => {
            ...
            Object.keys(
              this.displayBudgetValuesIndicator[partner_code][wp_id][item_id],
            ).forEach((indicator_id) => {
              if (
                this.summaryBudgetsIndicator[wp_id][item_id][indicator_id] ==
                null
              ) {
        ...
        Object.keys(this.perValues[partner_code][wp_id]).forEach((item_id) => {
          Object.keys(this.perValues[partner_code][wp_id][item_id]).forEach(
            (per_id) => {
              ...
              if (
                this.perValues[partner_code][wp_id][item_id][per_id] == true
              ) {
                this.perValuesSammary[wp_id][per_id] = true;
                this.perValuesSammaryForPartner[partner_code][wp_id][per_id] =
                  true;
              }
            },
          );
  ...
  checkEOI(category: any) {
    ...
    else return this.phase?.show_eoi ? category == 'EOI' : false;
  }
  ...
  async getDataForWp(
    ...
    let wp_data;
    if (
      wp_category != 'Projects' &&
      wp_category != 'Geographic-Scope' &&
      wp_category != 'partners' &&
      wp_category != 'melia' &&
      wp_category != 'Cross Cutting' &&
      wp_category != 'synergy-programs'
    ) {
      wp_data = this.results.filter((d: any) => {
        if (partner_code)
          return (
            ...
            (d.group == id ||
              ...
              // (d?.parent_id == id && d.category == 'Project' && wp_category == 'Projects') ||
              ((this.checkEOI(d.category) ||
                (d.category == 'OUTCOME' && !d.group)) &&
                ost_wp_acronym == 'AOW00') ||
              // ((this.checkEOI(d.category) || d.category == "Cross Cutting" || (!d.group && d.category != 'Melia') || (!d.parent_id && d.category == 'Melia')) && ost_wp_acronym == 'AOW00') ||
              d.wp_id == official_code)
            // (official_code == "CROSS" && this.checkEOI(d.category))
          );
      ...
      wp_data = this.results.filter((d: any) => {
        if (partner_code)
          return (
            d.category == 'Melia' &&
            (d?.parent_id == id ||
              (!d.group && d.category != 'Melia') ||
              (!d.parent_id &&
                d.category == 'Melia' &&
                ost_wp_acronym == 'CROSS'))
          );
        else
          return (
            d.category == 'Melia' &&
      ...
      wp_data = this.results.filter((d: any) => {
        if (partner_code)
          return (
            d.category == 'Cross Cutting' &&
            official_code == 'CROSS-Cross-Cutting'
          );
        else
          return (
            d.category == 'Cross Cutting' &&
  ...
  finalPeriodValForPartner(partner_code: number, period_id: any) {
    return this.actualWps
      .map(
        (wp: any) =>
          this.perValuesSammaryForPartner[partner_code][
            wp.ost_wp.wp_official_code
          ][period_id],
      )
      .reduce((a: any, b: any) => a || b);
  ...
  async getdata() {
    ...
    for (let submission of allSubmissions) {
      for (let tocData of emptyArray) {
        if (
          submission.initiative_id == tocData.initiative_id &&
          submission.phase.tocPhase == tocData.phase
        ) {
        ...
        if (typeof categoryData === 'object' && categoryData !== null) {
          Object.keys(categoryData).forEach((indicator) => {
            ...
            if (typeof value === 'number') {
              totals[partnerId][indicator] =
                (totals[partnerId][indicator] || 0) + value;
            }
          });
        }
      });
  ...
  getScope(indicator: any, type: string) {
    ...
    if (indicator[target] == 'global') {
      ...
      if (target == 'geographic_scope')
        scope =
          'Country: ' + indicator.country?.map((c: any) => c.name).join(', ');
      else
        scope =
          'Country: ' + indicator.countries?.map((c: any) => c.name).join(', ');
    } else if (indicator[target] == 'regional') {
      scope =
        'Regional: ' + indicator.regions?.map((c: any) => c.name).join(', ');
    }
    return scope;
  }
  getTargetValue(targets: any[], code: string = '') {
    ...
    if (code != '')
      filterd = targets.filter((target: any) =>
        target?.centers?.map((d: any) => d.code).includes(Number(code)),
      );
    else filterd = targets;

    return filterd.reduce((sum, target) => {
      const val =
        parseFloat(
          target?.[
            this?.phase?.reportingYear ||
              this?.submission_data?.phase?.reportingYear
          ],
        ) || 0;
      return sum + val;
    }, 0);
      ...
      if (wp.category === 'WP') {
        ...
        for (let data of allData) {
          ...
          for (let key of this.indicatorTypes) {
            this.perAllValuesIndicator[group][dataId][key] =
  ...
  async setAnaplanValues() {
    this.anaplanValues = await this.anaplanService.findAllValues(
      this.initiative_data.id,
      this.phase.id,
    );
    for (let values of this.anaplanValues) {
      this.anaplanBudgets[values.organization.code][
        values.workPackage.wp_official_code
      ][values.anaplan.id] = values.value;
    }
  }
  async setAnaplanValuesVersion(id: number) {
    this.anaplanValues = await this.anaplanService.findAllValuesVersion(
      this.initiative_data.id,
      id,
  ...
  async getActualTocs(code: string) {
    try {
      const response = await firstValueFrom(
        this.httpService.get(`${process.env.TOC_API}/toc/${code}`).pipe(
          catchError((error: AxiosError) => {
            console.error('TOC API Error:', error.message);
            throw new InternalServerErrorException('Failed to fetch TOC data');
          }),
        ),
      );
  ...
  async generateExcelProject(dataa: any, partner: any, type: string) {
    ...
    if (indicatorTogelvalue?.value == '1')
      data = dataa.filter((d: any) => {
        if (d?.center?.code && d?.center?.code == partner.code) return true;
        if (d?.center?.code && d?.center?.code != partner.code) return false;
    ...
    let totalLabel = '';
    if (type == 'project') totalLabel = 'W3/Bilateral projects subtotal';
    else totalLabel = 'MELIA Studies Budget subtotal';

    XLSX.utils.sheet_add_aoa(ws, [[totalLabel, null, null, null, null]], {
      origin: -1,
    });
  ...
  flattenProjectData(data: any[], partner: any, type: string) {
    ...
    for (const item of data) {
      ...
      const budgetEndCell = XLSX.utils.encode_cell({
        r: startRow + resultCount - 1,
        c: 3,
      });
  ...
  generateExcelAnaplan(partner: any) {
    ...
    this.anaplanLabels.forEach((anaplan) => {
      ...
      const endCell = XLSX.utils.encode_cell({
        r: currentRow,
        c: TOTAL_BUDGET_COLUMN_INDEX - 1,
      });
      const totalBudgetCell = XLSX.utils.encode_cell({
        r: currentRow,
        c: TOTAL_BUDGET_COLUMN_INDEX,
      });
  ...
  generateExcelSummaryAnaplan() {
    ...
    this.anaplanLabels.forEach((anaplan) => {
      ...
      this.actualWps.forEach((wp, colIndex) => {
        const budgetValue =
          this.getAnaplanValueAcrossPartners(
            wp.ost_wp.wp_official_code,
            anaplan.id,
          ) || 0;
        rowArray.push(budgetValue);
  ...
  setvaluesForIndicators(data: any[], index: number) {
    const indicatorIds =
      this.results[this.results.length - index].indicator_ids;
    ...
    for (let value of filtered) {
      ...
      this.displayBudgetValuesIndicator[org][wp][parent][result] = Number(
        value.budget,
      );
    }
  ...
  setPartnervaluesForIndicators(data: any[], index: number) {
    ...
    for (let value of filtered) {
      ...
      if (
        ...
        typeof this.budgetValuesIndicatorPartner[orgCode] !== 'object'
      ) {
        this.budgetValuesIndicatorPartner[orgCode] = {};
      }
      ...
      if (
        ...
        typeof this.budgetValuesIndicatorPartner[orgCode][wpCode] !== 'object'
      ) {
        this.budgetValuesIndicatorPartner[orgCode][wpCode] = {};
      }
      ...
      this.budgetValuesIndicatorPartner[orgCode][wpCode][indicatorType] +=
        budget;
  ...
  generateExcelSummaryConsolidated() {
    ...
    this.actualWps.forEach((wp) => {
      const row = [
        ...
        this.budgetValuesIndicatorSummary?.[wp.ost_wp.wp_official_code]?.[
          'Number of innovations (innovation development)'
        ] ?? 0,
        this.totalTargetsIndicator?.[wp.ost_wp.wp_official_code]?.[
          'Number of knowledge products'
  ...
  generateExcelCenterConsolidated(partner_code: number) {
    ...
    this.actualWps.forEach((wp) => {
      const row = [
        wp.title,
        this.totalTargetsIndicatorPartners?.[partner_code]?.[
          wp.ost_wp.wp_official_code
        ]?.['Number of innovations (innovation development)'] ?? 0,
        this.budgetValuesIndicatorPartner[partner_code]?.[
          wp.ost_wp.wp_official_code
        ]?.['Number of innovations (innovation development)'] ?? 0,
        this.totalTargetsIndicatorPartners?.[partner_code]?.[
          wp.ost_wp.wp_official_code
        ]?.['Number of knowledge products'] ?? 0,
        this.budgetValuesIndicatorPartner[partner_code]?.[
          wp.ost_wp.wp_official_code
        ]?.['Number of knowledge products'] ?? 0,
        this.totalTargetsIndicatorPartners?.[partner_code]?.[
          wp.ost_wp.wp_official_code
        ]?.['Number of people trained (capacity sharing for development)'] ?? 0,
        this.budgetValuesIndicatorPartner[partner_code]?.[
          wp.ost_wp.wp_official_code
        ]?.['Number of people trained (capacity sharing for development)'] ?? 0,
        this.totalTargetsIndicatorPartners?.[partner_code]?.[
        ...
        this.roundNumber(
          this.wp_budgets[partner_code][
            wp.ost_wp.wp_official_code + '-partners'
          ],
        ) ?? 0,
        this.roundNumber(
          this.wp_budgets[partner_code][wp.ost_wp.wp_official_code + '-melia'],
        ...
        wpItems.forEach((item, itemIndex) => {
          if (item.category == 'OUTPUT') {
            ...
            if (
              ...
              item.quantitative_indicators.length > 0
            ) {
                ...
                (indicator, indicatorIndex) => {
                  ...
                  if (isFirstIndicator) {
                    row = [
                      ...
                      this.summaryBudgetsIndicator[wpCode]?.[item.id]?.[
                        indicator?.id
                      ] || 0,
              ...
              if (numIndicators > 1) {
                merges.push({
                  s: { r: startRowForItem, c: 1 },
                  e: { r: startRowForItem + numIndicators - 1, c: 1 },
                });
                merges.push({
                  s: { r: startRowForItem, c: 7 },
                  e: { r: startRowForItem + numIndicators - 1, c: 7 },
                });
              }
            }
            // else  {
  ...
  generateExcelCenterHLO(partner_code: number) {
    console.log('generateExcelCenterHLO', partner_code);
    const headerStyle = {
      font: { bold: true, sz: 11, color: { rgb: 'FFFFFF' } },
      alignment: { horizontal: 'center', vertical: 'center', wrapText: true },
      fill: { fgColor: { rgb: '2B3C53' } },
    ...
    this.actualWps.forEach((wp) => {
      ...
      validItems.forEach((item) => {
        if (item.category == 'OUTPUT') {
          ...
          if (
            ...
            const totalBudgetFormula = `SUM(G${totalBudgetCellRow}:G${
              totalBudgetCellRow + item.quantitative_indicators.length - 1
            })`;
      ...
      if (currentRowIndex > wpStartRow) {
        ...
        const subtotalFormula = `SUM(H${subtotalFirstDataRow + 1}:H${
          subtotalLastDataRow + 1
        })`;
  ...
  generateExcelSummaryOutcome() {
    ...
    ws_data.push([null, null, 'Type', 'Geographic Location', 'Target']);
    ...
    this.actualWps.forEach((wp) => {
      const wpCode = wp.ost_wp.wp_official_code;
      const wpItems = this.allData[wpCode] || [];
      const isValidItem = wpItems.filter(
        (d) => d.category == 'OUTCOME' || d.category == 'EOI',
      );
  ...
  generateExcelSummaryPartner() {
    ...
    this.actualWps.forEach((wp) => {
      ...
      const validPartners = wpItems.filter(
        (item) => item.selectedCountries && item.selectedCountries.length > 0,
      );
      ...
      validPartners.forEach((item) => {
        ...
        if (item.selectedCountries && item.selectedCountries.length > 0) {
          ...
          if (numCountries > 1) {
            merges.push({
              s: { r: startRowForItem, c: 1 },
              e: { r: startRowForItem + numCountries - 1, c: 1 },
            });
            // Merge budget column (E)
            merges.push({
              s: { r: startRowForItem, c: 4 },
              e: { r: startRowForItem + numCountries - 1, c: 4 },
            });
          }
        }
      });
  ...
  generateExcelSummaryCrossCutting() {
    ...
    ws_data.push(['AOW', 'Cost elements', 'Total budget (USD)']);
