...
export class W3SectionComponent implements OnChanges {
  ...
  get outputOptions(): string[] {
    return Array.from(
      new Set(this.rows.map((row) => row.bilateral_outputs).filter(Boolean))
    ).sort();
  }
  ...
  get filteredRows() {
    ...
    return this.rows.filter((row) => {
      const matchesSearch =
        !search ||
        String(row.bilateral_name || "").toLowerCase().includes(search) ||
        String(row.bilateral_outputs || "").toLowerCase().includes(search) ||
        String(row.bilateral_assumption || "").toLowerCase().includes(search);
      const matchesOutputs =
        !this.filterOutputs || row.bilateral_outputs === this.filterOutputs;
      return matchesSearch && matchesOutputs;
    });
  }

  get subtotal(): string {
    const total = this.filteredRows.reduce((sum, r) => sum + (Number(r.bilateral_budget) || 0), 0);
  ...
  rowHasValidationError(row: any): boolean {
    const budget = this.parseBudgetValue(row?.bilateral_budget);
    return budget != null && budget > 0 && !String(row?.bilateral_assumption ?? "").trim();
  }
  ...
  async deleteRow(row: any) {
    ...
    try {
      const result = await this.porbService.deleteBilateral(row.id);
      if (result?.deleted) {
  ...
  export() {
    this.exportAsExcel(
      "w3-bilateral-projects.xls",
      ["Bilateral Name", "Outputs", "Budget"],
      this.filteredRows.map((row) => [
        row.bilateral_name,
        row.bilateral_outputs,
        row.bilateral_budget,
      ])
    );
  }
  ...
  private normalizeBudgetValues() {
    ...
    this.rows.forEach((row) => {
      if (row?.bilateral_budget === 0) {
        row.bilateral_budget = "";
      }
      row.bilateral_assumption = row.bilateral_assumption ?? "";
    });
  }
}
